...
public class BinaryUtilities : MonoBehaviour
{
    ...
    public static byte[] SerializeTransform(Transform t)
    {
        ...
        AddBytesToList(ref result, BitConverter.GetBytes(t.localEulerAngles.x));
        AddBytesToList(ref result, BitConverter.GetBytes(t.localEulerAngles.y));
        AddBytesToList(ref result, BitConverter.GetBytes(t.localEulerAngles.z));
    ...
    public static void DeSerializeTransformOnObject(ref byte[] data, int off, GameObject g)
    {
        ...
        g.transform.localEulerAngles = new Vector3(rotX, rotY, rotZ);
