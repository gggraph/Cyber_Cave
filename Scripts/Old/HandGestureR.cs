 ...
 */
    ...
    public void CreatePose(GameObject hand, byte handside)
    {
        ...
        BinaryUtilities.AddBytesToList(ref dataBytes, BitConverter.GetBytes(_Poses.Count));
        dataBytes.Add(handside);
        foreach ( OVRBone b in candidate)
        {
            BinaryUtilities.AddBytesToList(ref dataBytes, BitConverter.GetBytes(b.Transform.position.x));
            BinaryUtilities.AddBytesToList(ref dataBytes, BitConverter.GetBytes(b.Transform.position.y));
            BinaryUtilities.AddBytesToList(ref dataBytes, BitConverter.GetBytes(b.Transform.position.z));

            BinaryUtilities.AddBytesToList(ref dataBytes, BitConverter.GetBytes(b.Transform.localEulerAngles.x));
            BinaryUtilities.AddBytesToList(ref dataBytes, BitConverter.GetBytes(b.Transform.localEulerAngles.y));
            BinaryUtilities.AddBytesToList(ref dataBytes, BitConverter.GetBytes(b.Transform.localEulerAngles.z));
        }
