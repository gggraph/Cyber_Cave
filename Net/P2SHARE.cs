...
public class P2SHARE : MonoBehaviour
{
    ...
    public enum CustomFileType : byte
    {
        Default = 0,
        MeshFile = 1, 
        SaveFile = 2, 
    ...
    public static string GetDirByType(byte t)
    {
        switch (t)
        {
            ...
            case 5:
                return Application.persistentDataPath + "/sculpt/";
    ...
    // Clean DL files and inform Seeder
    public static void ForceEndAllDlls()
    {
        foreach ( DL dl in _dlls)
        {
            // Inform seeder
            SendDLLErrorToAll(dl.checksum);
            // Destroy file
            File.Delete(dl.filePath);

        }
        _dlls = new List<DL>();
    }
    public static DL RequestNewFile(string fileName, byte[] checksum, int fSize, byte type, NetStream.DefaultRPCMessage fromMessage = null)
    {
        if (GetDLByChecksum(ref checksum) != null)
            return null;

        string filePath = GetDirByType(type) + fileName;
        
        // Create Recipient
        File.WriteAllBytes(filePath, new byte[fSize]);
        ...
        data.Add((byte)fileName.Length); // at 34
        foreach (char c in fileName.ToCharArray())
            data.Add((byte)c);
        NetUtilities.SendDataToAll(data.ToArray());

        // Create DL object
        DL dl = new DL(checksum, (uint)fSize, type, filePath);
        _dlls.Add(dl);
    ...
    public static void SendDLLErrorToAll(byte[] chksm)
    {
        Debug.Log("Sending a DLL ERROR...");
        List<byte> data = new List<byte>();
        data.Add(15);
...
*/
       ...
       // {
            for (int i = 0; i < _ulls.Count; i++)
            {
                ...
                if (_ulls[i].current_offset >= _ulls[i].ini_offset + _ulls[i].target_len)
                {
                    ...
                    Debug.LogError("Uploading cell done..");
                ...
                Debug.Log("Sending Data . file : " + _ulls[i].filePath + " Offset : " + _ulls[i].current_offset);
    ...
    public static UL OnFileRequest(byte[] data, PhotonMessageInfo info)
    {
        ...
        */
        string fpath = GetDirByType(data[1]);
        // Get Name
        char[] fname = new char[data[34]];
        for(int i = 0; i < fname.Length; i++)
        {
            fname[i] = (char)data[i + 35];
        }
