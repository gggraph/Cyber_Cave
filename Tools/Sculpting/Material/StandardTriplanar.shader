    ...
    Properties
    {
        ...
        SubShader
        {
            ...
            void surf(Input IN, inout SurfaceOutputStandard o)
            {
                ...
                // Base color
                // Get color triplanar coordinate... 
                half4 cx = tex2D(_MainTex, tx) * bf.x;
                half4 cy = tex2D(_MainTex, ty) * bf.y;
                half4 cz = tex2D(_MainTex, tz) * bf.z;

                // combine colors of all three planar CX CY CZ into one. Combine also with main color (teint) and vertex color.
                half4 col = (cx + cy + cz) * _Color;
                col = lerp(col, IN.vertColor, IN.vertColor.a);
                col.a = 1;
                o.Albedo = col.rgb;
                o.Alpha = col.a;
