...
public class Souvenir : MonoBehaviour
{
   ...
   public IEnumerator SouvenirRoutine() 
   {
    ...
    public bool PlayRecordGestureFromTimeStamp(GameObject go, uint TimeStamp)
    {
        ...
        string[] rcFiles = Directory.GetFiles(Application.persistentDataPath + "/" + go.GetComponent<DataReceiver>()._nickname);
    ...
    public uint DejaVu(GameObject go)
    {
        ...
        for (int i = 0; i < Players.Length; i++)
        {
            ...
            string[] rcFiles = Directory.GetFiles(Application.persistentDataPath + "/" + Players[i].GetComponent<DataReceiver>()._nickname);
            foreach (string s in rcFiles)
            {
