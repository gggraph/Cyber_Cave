...
public class CryptoUtilities : MonoBehaviour
{
    ...
    public static byte[] GetUniqueHashFromViewID(string seed, PhotonView pw)
    {
        List<byte> tohash = new List<byte>();
        foreach (char c in seed.ToCharArray())
            tohash.Add((byte)c);
        tohash.AddRange(BitConverter.GetBytes(Utilities.GetTimeStamp()));
        tohash.AddRange(BitConverter.GetBytes(pw.ViewID));
        return ComputeSHA(tohash.ToArray());
